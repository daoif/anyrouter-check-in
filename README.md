...
- ✅ 多种机器人通知（可选）
- ✅ 绕过 WAF 限制
- ✅ 内置配置管理器（浏览器本地存储，安全便捷）

## 配置管理器

项目内置了一个配置管理器页面 (`docs/index.html`)，可以方便地管理账号、更新 Cookie、配置通知渠道。

**使用方式：**

- **本地打开：** 直接用浏览器打开项目中的 `docs/index.html` 文件即可
- **GitHub Pages：** Fork 后开启 GitHub Pages（source 选择 `docs` 目录），即可在线访问
...
- 编辑即自动保存，下次打开自动恢复
- 支持导入/导出 JSON 备份文件
- 一键生成所有 GitHub Secrets 配置，复制粘贴即可
...
对于每个需要签到的账号，你需要获取：(推荐使用 [配置管理器](#配置管理器) 来编辑和管理)
